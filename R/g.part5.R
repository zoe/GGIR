      ...
      if (length(idindex) > 0 & nrow(summarysleep) > 1) { #only attempt to load file if it was processed for sleep
        ...
        for (j in def) { # loop through sleep definitions (defined by angle and time threshold in g.part3)        
          ...
          if (nrow(summarysleep_tmp2) > 0) {
            ...
            write.csv(ACC_SPT,file="/home/vincent/accsptexmaple.csv",row.names = FALSE)
            ...
            # LIDS = (10*(60/ws3)) / (bincla2 + 1) #max value devided by (score + 1)
            # LIDS2 = zoo::rollmean(x=LIDS,k=(60*30)/ws3) # 30 minute rolling average
            # 
            # x11()
            # par(mfrow=c(2,1))
            # plot(LIDS,type="l",main="Figure 1E")
